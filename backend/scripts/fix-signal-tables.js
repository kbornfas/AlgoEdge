...
async function checkAndFix() {
  ...
  try {
    ...
    // First ensure signal_tiers exists
    await client.query(`
    ...
    `);
    console.log('✅ signal_tiers table ready');
    
    // Check if tiers exist
    const tiersCount = await client.query('SELECT COUNT(*) FROM signal_tiers');
    if (parseInt(tiersCount.rows[0].count) === 0) {
      console.log('Inserting signal tiers...');
      await client.query(`
    ...
    const masterCount = await client.query('SELECT COUNT(*) FROM master_accounts');
    if (parseInt(masterCount.rows[0].count) === 0) {
      await client.query(`
        INSERT INTO master_accounts (name, account_id, broker, login, server)
        VALUES ('AlgoEdge Main', '12345', 'AlgoEdge', 'main', 'AlgoEdge-Live')
      `);
    }
    console.log('✅ master_accounts table ready');
    
    // Rename old signal_subscriptions if it exists with wrong schema
    ...
    if (cols.rows.length > 0 && !hasUserId) {
      console.log('Renaming old signal_subscriptions to signal_provider_subscriptions...');
      await client.query('ALTER TABLE signal_subscriptions RENAME TO signal_provider_subscriptions');
    }
    
    // Create our signal_subscriptions table
    await client.query(`
    ...
    `);
    console.log('✅ signal_subscriptions table ready');
    
    // Create trading_signals
    await client.query(`
    ...
    `);
    console.log('✅ trading_signals table ready');
    
    // Create signal_deliveries
    await client.query(`
    ...
    `);
    console.log('✅ signal_deliveries table ready');
    
    // Create signal_channels
    await client.query(`
    ...
    `);
    console.log('✅ signal_channels table ready');
    
    // Create indexes
    await client.query('CREATE INDEX IF NOT EXISTS idx_signal_subs_user ON signal_subscriptions(user_id)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_signal_subs_status ON signal_subscriptions(status)');
