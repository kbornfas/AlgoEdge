...
// Request seller verification ($50)
router.post('/seller/request-verification', authenticate, async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    await pool.query(
      'UPDATE users SET verification_pending = TRUE WHERE id = $1',
      [userId]
    );

    auditLog(userId, 'SELLER_VERIFICATION_REQUESTED', { fee: VERIFICATION_FEE }, req);

    res.json({ 
      success: true, 
      message: 'Verification request submitted. Your account will be reviewed by our team within 24-48 hours.',
      pendingReview: true
    });
  } catch (error) {
